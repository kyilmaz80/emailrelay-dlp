...
class SocketThread(Thread):
    ...
    def recieve(self):
        ...
        self.msg_path = get_msg_fpath(msg_fpath, params.EML_MOUNT)
...
# load patterns
patterns = load_patterns(params.PATTERN_FILE)
...
try:
    ...
    logger.info('%s:%s bind done', params.HOST, params.PORT)
except Exception as e:
    logger.error('%s:%s bind failure!', params.HOST, params.PORT)
    sys.exit(1)
...
while True:
    logger.info('------------------------------------------')
    logger.info('Server is waiting for data on port: %s', params.PORT)
